  ...
  return (
    <div>
      ...
      <div className="relative">
        <input id={id} name={id} type={visible ? "text" : "password"} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholders[id]} required autoComplete={id === "currentPassword" ? "current-password" : "new-password"} className="h-11 w-full rounded border border-slate-300 bg-white px-3 pr-11 text-sm outline-none placeholder:text-slate-400 focus:border-[#2674b7] focus:ring-2 focus:ring-[#2674b7]/15" />
...
const changePassword = async (values: ChangePasswordInput): Promise<string> => {
  ...
  const payload: { message?: string; data?: { message?: string } } = await response
    .json()
    .catch(() => ({}));
  ...
  return payload.data?.message || payload.message || "Password changed successfully.";
};
...
const ChangePasswordContainer = () => {
  ...
  const rules = [
    { valid: newPassword.length >= 8, label: "Minimum 8–12 characters (recommend 12+ for stronger security)." },
    { valid: /[A-Z]/.test(newPassword), label: "At least one uppercase letter must." },
    { valid: /[a-z]/.test(newPassword), label: "At least one lowercase letter must." },
    { valid: /\d/.test(newPassword), label: "At least one number must (0–9)." },
    { valid: /[^A-Za-z0-9\s]/.test(newPassword), label: "At least special character (! @ # $ % ^ & * etc.)." },
    { valid: newPassword.length > 0 && !/\s/.test(newPassword), label: "No spaces allowed." },
  ];
        ...
        <div className="mt-10 grid items-start gap-5 rounded-xl border border-slate-200 bg-white p-3 shadow-sm sm:mt-12 sm:p-4 lg:grid-cols-[280px_minmax(0,1fr)]">
          ...
          <div className="min-h-[500px] rounded-lg border border-slate-200 bg-[#f8f9fa] p-5 sm:p-8 lg:p-10">
            ...
            <form onSubmit={handleSubmit} className="mt-8">
              ...
              <div className="mt-5"><PasswordField id="confirmPassword" label="Confirm New Password" value={confirmPassword} onChange={setConfirmPassword} /></div>
              ...
              <div className="mt-8 flex flex-col justify-end gap-3 sm:flex-row">
                ...
                <button type="submit" disabled={mutation.isPending} className="h-10 rounded-full bg-[#2a73b5] px-6 text-sm font-medium text-white transition hover:bg-[#205f96] disabled:cursor-not-allowed disabled:opacity-60">{mutation.isPending ? "Saving..." : "Save Changes"}</button>
              </div>
            </form>
          </div>
        </div>
